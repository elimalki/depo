...
/**
 * wrap BLE communication into socket like class - connect, disconnect and write as methods, - read
 * + status is returned by SerialListener
 */
class SerialSocket extends BluetoothGattCallback {

  private static final UUID BLUETOOTH_LE_BM71_SERVICE = UUID
      .fromString("49535343-FE7D-4AE5-8FA9-9FAFD205E455");
  private static final UUID BLUETOOTH_LE_BM71_CHAR_RW = UUID
      .fromString("49535343-1E4D-4BD9-BA61-23C647249616");
  private static final UUID BLUETOOTH_LE_CCCD = UUID
   ...
   */
  void connect(Context context, SerialListener listener, BluetoothDevice device)
      throws IOException {
  ...
  public void onServicesDiscovered(BluetoothGatt gatt, int status) {
    ...
    for (BluetoothGattService gattService : gatt.getServices()) {
      if (gattService.getUuid().equals(BLUETOOTH_LE_BM71_SERVICE)) {
        readCharacteristic = gattService.getCharacteristic(BLUETOOTH_LE_BM71_CHAR_RW);
        writeCharacteristic = gattService.getCharacteristic(BLUETOOTH_LE_BM71_CHAR_RW);
      }
    }
    ...
    if ((writeProperties & (BluetoothGattCharacteristic.PROPERTY_WRITE +
        // Microbit,HM10-clone have WRITE
        BluetoothGattCharacteristic.PROPERTY_WRITE_NO_RESPONSE))
        == 0) { // HM10,TI uart have only WRITE_NO_RESPONSE
    ...
    if ((readProperties & BluetoothGattCharacteristic.PROPERTY_INDICATE) != 0) {
      ...
      onSerialConnectError(new IOException(
          "no indication/notification for read characteristic (" + readProperties + ")"));
      return;
    }
